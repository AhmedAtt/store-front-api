...
export class OrdersModel {
  ...
  async create(o: OrderWithItems): Promise<OrderWithItems> {
    try {
      ...
      for (const item of o.order_items) {
        const { rows } = await conn.query(orderItemsSql, [
          order.id,
          item.product_id,
          item.quantity,
        ]);
  ...
  async showActiveOrder(userId: number): Promise<OrderWithItems> {
    try {
      ...
      throw new Error(
        `Could not find active order for user ${userId}. Error: ${err}`
      );
    }
  }
  ...
  async showCompletedOrders(userId: number): Promise<OrderWithItems[]> {
    try {
      ...
      for (const order of orders) {
        const { rows: orderItems } = await conn.query(orderItemsSql, [
          order.id,
        ]);
      ...
      throw new Error(
        `Could not find completed order for user ${userId}. Error: ${err}`
      );
    }
  }
}
